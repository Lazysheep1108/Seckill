...
public class UserUtil {
    public static void create(int count) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //小伙伴也可以生成不同的密码
            user.setPassword(MD5Util.inputPassToDBPass("123", user.getSlat()));
            users.add(user);
        }
        ...
        File file = new File("C:\\SoftWare\\config.txt");
        if (file.exists()) {
            file.delete();
        }
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            String params = "mobile=" + user.getId() + "&password=" + MD5Util.inputPassToMidPass("123");
